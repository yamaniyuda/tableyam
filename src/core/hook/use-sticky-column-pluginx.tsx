...
export function columnIsBeforeRightSticky(columnId: Column['id'], columns: any): boolean {
  const index = columns.findIndex(({ id }: any) => id === columnId);
  const prevColumn = columns[index + 1];
  const columnIsRightSticky = getStickyValue(prevColumn) === 'right';
  return columnIsRightSticky
}
...
export function getMarginRight(columnId: Column['id'], columns: any) {
  ...
  for (let i = currentIndex + 1; i < columns.length; i ++) {
    if (columns[i].isVisible !== false) {
      let width = parseInt(columns[i].width.slice(0, columns[i].width.length - 1))
...
function getStickyProps(header: any, instance: any) {
  let style: Object = {width: `${header.width}`};
  const dataAttrs = {};
...
export const useStickyColumnPlugin = (hooks: any) => {
...
useStickyColumnPlugin.pluginName = 'useStickyColumnPlugin';